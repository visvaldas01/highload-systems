...
public class Recommendation {
    ...
    private Collection<Song> history;
    ...
    public Recommendation(Collection<Song> library, SongRepository songRep)
    {
        ...
        for (Song s: history) {
            if (s.getVector1() < MinV1)
                MinV1 = s.getVector1();
            if (s.getVector2() < MinV2)
                MinV2 = s.getVector2();
            if (s.getVector3() < MinV3)
                MinV3 = s.getVector3();

            if (s.getVector1() > MaxV1)
                MaxV1 = s.getVector1();
            if (s.getVector2() > MaxV2)
                MaxV2 = s.getVector2();
            if (s.getVector3() > MaxV3)
                MaxV3 = s.getVector3();
        }
    }
